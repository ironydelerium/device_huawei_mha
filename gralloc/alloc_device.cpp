...
static int gralloc_alloc_framebuffer_locked(alloc_device_t* dev, size_t size, int usage, buffer_handle_t* pHandle, int* stride, int* byte_stride)
{
	...
	AINF("gralloc_alloc_framebuffer_locked(): framebufferSize = %zu, alignedFramebufferSize = %zu", framebufferSize, alignedFramebufferSize);
	...
	*stride = m->info.xres;
	...
	{
		AERR("gralloc_alloc_framebuffer_locked(): bufferMask = %u, numBuffers = %u, out of buffers", bufferMask, numBuffers);
	...
	*byte_stride = m->finfo.line_length;
    ...
    AINF("gralloc_alloc_framebuffer_locked(): output handle %p, stride %d, byte_stride %d", hnd, *stride, *byte_stride);
	return 0;
}

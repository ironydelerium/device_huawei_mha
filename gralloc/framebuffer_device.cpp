...
#define NUM_BUFFERS 2 /* NUM_FB_BUFFERS  */
...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
	...
	{
		AERR("invalid private_handle_t in %s", __FUNCTION__);
		return -EINVAL;
	}
...
int init_frame_buffer_locked(struct private_module_t* module)
{
	...
	if (fd < 0)
	{
		AERR("can't open framebuffer: %s", strerror(errno));
		return -errno;
	}
	...
	{
		AERR("FBIOGET_FSCREENINFO failed: %s", strerror(errno));
		return -errno;
	}
	...
	{
		AERR("FBIOGET_VSCREENINFO failed: %s", strerror(errno));
		return -errno;
	}
	...
	if (ioctl(fd, FBIOGET_VSCREENINFO, &info) == -1)
	{
		AERR("FBIOGET_VSCREENINFO [2] failed: %s", strerror(errno));
		return -errno;
	}
...
/*
	if ( info.pixclock > 0 )
	{
		...
		AINF("fbdev vertical: %d, horizontal: %d, pixclock = %d, refreshRate = %d", 
...
*/
	...
	if (ioctl(fd, FBIOGET_FSCREENINFO, &finfo) == -1)
	{
		AERR("FBIOGET_FSCREENINFO [2] failed: %s", strerror(errno));
		return -errno;
	}

	if (finfo.smem_len <= 0)
	{
		AERR("finfo.smem_len overflow: %d", finfo.smem_len);
		return -errno;
	}
...
int framebuffer_device_open(hw_module_t const* module, const char* name, hw_device_t** device)
{
	...
	if (status < 0)
	{
		AERR("gralloc_open() failed: %d", status);
		return status;
	}
	...
	{
		if (dev) 
			AERR("framebuffer_device_open(): init_frame_buffer() failed: %d", status);
		else
			AERR("framebuffer_device_open(): allocating framebuffer_device_t object failed");

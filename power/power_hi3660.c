...
static void interactive_power_init(struct hi3660_power_module __unused *hi3660)
{
...
static int interactive_boostpulse(struct hi3660_power_module *hi3660)
{
    char buf[80];
    int len;

   if (hi3660->boostpulse_fd < 0)
        hi3660->boostpulse_fd = open(INTERACTIVE_BOOSTPULSE_PATH, O_WRONLY);

    if (hi3660->boostpulse_fd < 0) {
        if (!hi3660->boostpulse_warned) {
            strerror_r(errno, buf, sizeof(buf));
            ALOGE("Error opening %s: %s\n", INTERACTIVE_BOOSTPULSE_PATH,
                      buf);
            hi3660->boostpulse_warned = 1;
        }
        return hi3660->boostpulse_fd;
    }

    len = write(hi3660->boostpulse_fd, "1", 1);
    if (len < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error writing to %s: %s\n",
...
static void
hi3660_devfreq_set_interactive(struct hi3660_power_module __unused *hi3660, int on)
{
...
static void hi3660_devfreq_init(struct hi3660_power_module __unused *hi3660)
{
...
int schedtune_sysfs_boost(struct hi3660_power_module *hi3660, char* booststr)
{
    char buf[80];
    int len;

    if (hi3660->schedtune_boost_fd < 0)
        return hi3660->schedtune_boost_fd;

    len = write(hi3660->schedtune_boost_fd, booststr, strlen(booststr));
    if (len < 0) {
        strerror_r(errno, buf, sizeof(buf));
...
{
    struct hi3660_power_module *hi3660 = (struct hi3660_power_module *)arg;

    while(1) {
        sem_wait(&hi3660->signal_lock);
        while(1) {
            long long now, sleeptime = 0;

            pthread_mutex_lock(&hi3660->lock);
            now = gettime_ns();
            if (hi3660->deboost_time > now) {
                sleeptime = hi3660->deboost_time - now;
                pthread_mutex_unlock(&hi3660->lock);
                nanosleep_ns(sleeptime);
                continue;
            }

            schedtune_sysfs_boost(hi3660, schedtune_boost_norm);
            hi3660_devfreq_set_interactive(hi3660, 0);
            hi3660->deboost_time = 0;
            pthread_mutex_unlock(&hi3660->lock);
            break;
        }
    }
...
static int schedtune_boost(struct hi3660_power_module *hi3660)
{
    long long now;
    ...
    now = gettime_ns();
    if (!hi3660->deboost_time) {
        schedtune_sysfs_boost(hi3660, schedtune_boost_interactive);
        hi3660_devfreq_set_interactive(hi3660, 1);
        sem_post(&hi3660->signal_lock);
    }
    hi3660->deboost_time = now + schedtune_boost_time_ns;

    return 0;
}

static void schedtune_power_init(struct hi3660_power_module *hi3660)
{
    char buf[50];
    pthread_t tid;

    hi3660->deboost_time = 0;
    sem_init(&hi3660->signal_lock, 0, 1);

    hi3660->schedtune_boost_fd = open(SCHEDTUNE_BOOST_PATH, O_RDWR);
    if (hi3660->schedtune_boost_fd < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error opening %s: %s\n", SCHEDTUNE_BOOST_PATH, buf);
        return;
    ...
    if (hi3660->schedtune_boost_fd >= 0) {
        size_t len = read(hi3660->schedtune_boost_fd, schedtune_boost_norm,
                          PROPERTY_VALUE_MAX);
	if (len <= 0)
    ...
    pthread_create(&tid, NULL, schedtune_deboost_thread, hi3660);
}

/*[generic functions]*********************************************************/

static void hi3660_cpufreq_set_interactive(struct power_module __unused *module, int on)
{
    int i;
    ...
    for (i=0; i < max_clusters; i++) {
        if ((!on || low_power_mode) && hi3660_cpufreq_clusters[i].low_power_max[0] != '\0')
		sysfs_write(hi3660_cpufreq_clusters[i].path, hi3660_cpufreq_clusters[i].low_power_max);
	else
		sysfs_write(hi3660_cpufreq_clusters[i].path, hi3660_cpufreq_clusters[i].normal_max);
    }
    sysfs_write(INTERACTIVE_IO_IS_BUSY_PATH, on ? "1" : "0");
}


static void hi3660_cpufreq_init(struct hi3660_power_module __unused *hi3660)
{
    char buf[128];
    int len, i;
    ...
    for (i=0; i < NR_CLUSTERS; i++) {
        ...
        property_get(buf, hi3660_cpufreq_clusters[i].path, "");

        if (hi3660_cpufreq_clusters[i].path[0] == '\0') {
            if (i == 0) {
                /* In case no property was set, pick cpu0's cluster */
                strncpy(hi3660_cpufreq_clusters[i].path,
        ...
        property_get(buf, hi3660_cpufreq_clusters[i].low_power_max, "");
        len = sysfs_read(hi3660_cpufreq_clusters[i].path,
                         hi3660_cpufreq_clusters[i].normal_max,
                         PROPERTY_VALUE_MAX);
        ALOGV("Cluster: %d path: %s  low: %s norm: %s\n", i,
              hi3660_cpufreq_clusters[i].path,
              hi3660_cpufreq_clusters[i].low_power_max,
              hi3660_cpufreq_clusters[i].normal_max);
    }
    max_clusters = i;
}

static void hi3660_power_init(struct power_module __unused *module)
{
    struct hi3660_power_module *hi3660 = container_of(module,
                                              struct hi3660_power_module, base);
    hi3660_cpufreq_init(hi3660);
    hi3660_devfreq_init(hi3660);
    interactive_power_init(hi3660);
    schedtune_power_init(hi3660);
}

static void hi3660_hint_interaction(struct hi3660_power_module *mod)
{
...
static void hi3660_power_hint(struct power_module *module, power_hint_t hint,
                                void *data)
{
    struct hi3660_power_module *hi3660 = container_of(module,
                                              struct hi3660_power_module, base);
    ...
    switch (hint) {
     case POWER_HINT_INTERACTION:
        hi3660_hint_interaction(hi3660);
        break;
   ...
   case POWER_HINT_VSYNC:
    ...
    case POWER_HINT_LOW_POWER:
        low_power_mode = data;
        hi3660_cpufreq_set_interactive(module, 1);
        break;
                    ...
                    hw_device_t** device)
{
    ...
    if (strcmp(name, POWER_HARDWARE_MODULE_ID) == 0) {
        struct hi3660_power_module *dev = (struct hi3660_power_module *)calloc(1,
                sizeof(struct hi3660_power_module));
        ...
        if (dev) {
            ...
            dev->base.init = hi3660_power_init;
            dev->base.powerHint = hi3660_power_hint;
            dev->base.setInteractive = hi3660_cpufreq_set_interactive;
...
struct hi3660_power_module HAL_MODULE_INFO_SYM = {
    .base = {
        .common = {
